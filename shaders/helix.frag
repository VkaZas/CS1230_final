...
float calcDisplacement(in vec3 p) {
   ...
   //return p.x*n.x + p.y*n.y + p.z*n.z;
    //return p.x*n.y + p.y*n.z + p.z*n.x;
...
{
    vec3 res = vec3(0.);
    vec4 colorx = texture(sam, p.yz) * abs(n.x);
    vec4 colory = texture(sam, p.xz) * abs(n.y);
    vec4 colorz = texture(sam, p.xy) * abs(n.z);
    res = vec3(colorx + colory + colorz);
    return res;
}
...
PrimitiveDist map(vec3 p) {
    ...
    if (sdFloor(p) < sdTwistedSphere(p)) {
        return PrimitiveDist(sdFloor(p), PLANE);
    } else {
        return PrimitiveDist(sdTwistedSphere(p), SPHERE);
    }
}
